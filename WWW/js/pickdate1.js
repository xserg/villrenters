//var aurl = 'http://www.rentalsystems.com/advert_price.asp?ref=';
...
function check_send(){
	...
	with(document.forms.calendar){
			...
			action = aurl+"&ck=ck&req1=" + req1.value + "&req2=" + req2.value +"&rcam=" + document.forms.calendar.dscode.value + "&ctype=";
...
with(document.forms.calendar){
	//alert(document.forms.calendar["ds1[d]"].type);
		if(date1.value == '' && date2.value == ''){
			date1.value = y1 + "/" + m1 +"/" + d1; //pick;
			req1.value = pdate;
			document.forms.calendar["ds1[d]"].selectedIndex = parseInt(d1);
			document.forms.calendar["ds1[F]"].selectedIndex = parseInt(m1);
			document.forms.calendar["ds1[Y]"].selectedIndex = parseInt(y1) - 2010;}
		else if(date1.value != '' && date2.value ==''){
			date2.value = y1 + "/" + m1 +"/" + d1; //pick;
			req2.value = pdate;
			document.forms.calendar["d2[d]"].selectedIndex = parseInt(d1);
			document.forms.calendar["d2[F]"].selectedIndex = parseInt(m1);
			document.forms.calendar["d2[Y]"].selectedIndex = parseInt(y1)- 2010;
			date_OK = true;
		...
		if (date_OK){
			numdays.value = days;
				//action = aurl+"&start="+ req1.value + "&end=" + req2.value + "&days=" + days + "&ctype=&rcam=" ;
		}
		//alert(action);
	}// end of with
