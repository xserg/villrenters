...
class Project_Countries_Index extends DVS_Dynamic
{
    ...
    private $lang_ru = array(
        'Self-catering_holiday_rentals_around_the_World' => 'Аренда вилл по всему миру',
    ...
    function getPageData()
    {
        ...
        while ($this->db_obj->fetch()) {
            ...
            //$сountries_obj->whereAdd("counter > 100");

            $сountries_obj->parent_id = $this->db_obj->id;
            $cnt = $сountries_obj->count();
            $сountries_obj->orderBy('rus_name');


            //$сountries_obj->find();
            //echo $cnt.'<br>';

            $сountries_obj->getList($this->db_obj->id);

            $i = 0;
            while ($сountries_obj->fetch()) {
                $country_link = 'regions/'.$сountries_obj->getAlias().'/';
                if ($сountries_obj->name == 'Moscow') {
                ...
                'COUNTRY_NAME' => $сountries_obj->getLocalName(),
        ...
        if ($this->role != 'mu') {
            $this->template_obj->setVariable(array(
                ...
                'image_header' => '<img src=/img/header/Header'.$img_arr[array_rand($img_arr)].'.jpg alt="аренда вилл" title="Аренда вилл, апартаментов, домов, коттеджей">',
        ...
        $page_arr['PAGE_TITLE'] = $this->layout_obj->project_title.' - Аренда вилл, апартаментов, домов, коттеджей';
    ...
    */
    ...
    function getAnnonce()
    {
        ...
        'ANNONCE' => nl2br(substr($pages_obj->body, 0, 490).'...').'<br><a href="/pages/about2">подробнее...</a>'));
    }
