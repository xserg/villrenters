 ...
 * Карточка кампании
 * @package web2matrix
 * $Id: Comments_Show.php 288 2013-08-22 14:06:42Z xxserg@gmail.com $
 */
...
*/
        ...
        $page_arr['PAGE_TITLE'] = 'Отзывы об аренде '.$villa_obj->title_rus.($this->region_title_t ? ' в '.$this->region_title_t : 'вилл, апартаментов, домов, квартир, коттеджей, шато, шале').' - Аренда жилья - '.$this->layout_obj->project_title;
    ...
    function setData()
    {
            $this->template_obj->setVariable(
                    array(
                        'VILLA_LINK'      => $this->db_obj->villa_id ? '/villa/'.$this->db_obj->villa_id.'.html' : $this->db_obj->link,
                        'VILLA_TITLE' => $this->db_obj->villa_id ? substr($this->db_obj->villa_title_rus, 0, 20) : $this->db_obj->title,
            ...
            if ($this->db_obj->rating) {
                $this->template_obj->setVariable(array(
                    ...
                    'Rating' => 'Рейтинг',
                    'approved' => 'проверено Villarenters.ru'
                ));
            }          
    }
